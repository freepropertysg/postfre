    ...
    <div class="nav-container">
      ...
      <a href="${base || "/"}" class="nav-logo" aria-label="Free Classified Ads – PostFre"
       style="display:flex;flex-direction:column;line-height:1.15;text-decoration:none;padding:4px 0;">
        ...
        <span style="font-size:12.5px;font-weight:400;font-family:Poppins,sans-serif;color:#6B7280;">
          Free Classified Ads
        </span>
      </a>
...
links.forEach(a => {
  ...
  // BLOG (index + articles)
  if (href.endsWith("/blog") && path.startsWith(href)) {
    a.classList.add("active");
    return;
  }

  // Exact match (ABOUT, LISTINGS, CONTACT)
  if (href === path) {
    a.classList.add("active");
  }
});
